var BLACKBYTES_ASSET = require('trustnote-common/constants').BLACKBYTES_ASSET;
var balances = require('trustnote-common/balances');
...
function initWallet() {
	...
	function setWalletNameAndColor(walletName) {
		...
		var color = root.config.colorFor ? root.config.colorFor[root.focusedClient.credentials.walletId] : '#20498b';
		if(!color) color = '#20498b';
	...
	function setWalletsInMenu() {
		...
		for (var key in root.walletClients) {
			...
			html += '<li onclick="wallet.selectWallet(\'' + walletId + '\')" id="w' + walletId + '" class="nav-item ' + (walletId === selectedWalletId ? 'selected' : '') + '">' +
				'<a class="oh"><div class="avatar-wallet " style="background-color: ' + (colors && colors[walletId] ? colors[walletId] : '#20498b') + '">' + credentials.walletName.substr(0, 1) + ' </div>' +
