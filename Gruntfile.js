module.exports = function(grunt) {
	...
	function getPlatform(){
		switch(process.platform){
			case 'win32': return 'win'; // change to 'win' for both 32 and 64
  ...
  grunt.initConfig({
    ...
    exec: {
      ...
      osx64: {
        command: '../trustnotebuilds/build-osx.sh osx64'
      },
      osx32: {
        command: '../trustnotebuilds/build-osx.sh osx32'
      }
    },
    watch: {
      options: {
    ...
    concat: {
      ...
      js: {
        ...
        dest: 'public/trustnote.js'
      },
      css: {
        src: ['src/css/*.css'],
        dest: 'public/css/trustnote.css'
      },
      foundation: {
        src: [
    ...
    uglify: {
      ...
      prod: {
        files: {
          'public/trustnote.js': ['public/trustnote.js'],
    ...
    copy: {
      ...
      osx: {
        ...
        dest: '../trustnotebuilds/'
      },
      linux: {
		options: {timestamp: true, mode: true},
        files: [
          {expand: true, cwd: './webkitbuilds/', src: ['trustnote.desktop', '../public/img/icons/icon-white-outline.ico', '../public/img/icons/icon-white-256.png'], dest: '../trustnotebuilds/Trustnote/linux32/', flatten: true, filter: 'isFile', options: {timestamp: true, mode: true} },
          {expand: true, cwd: './webkitbuilds/', src: ['trustnote.desktop', '../public/img/icons/icon-white-outline.ico', '../public/img/icons/icon-white-256.png'], dest: '../trustnotebuilds/Trustnote/linux64/', flatten: true, filter: 'isFile', options: {timestamp: true, mode: true} },
        ],
      }
    },
    karma: {
    ...
    nwjs: {
      options: {
          ...
          macPlist: {CFBundleIdentifier: 'org.trustnote.wallet', CFBundleURLTypes: [{CFBundleURLName: 'Trustnote action', CFBundleURLSchemes: ['trustnote']}], /*CFBundleIconFile: 'nw.icns',*/ LSHasLocalizedDisplayName: 0}
      },
    ...
    compress: {
      linux32: {
        options: {
          archive: '../trustnotebuilds/trustnote-linux32.zip'
        },
        expand: true,
        cwd: '../trustnotebuilds/Trustnote/linux32/',
        src: ['**/*'],
        dest: 'trustnote-linux32/'
      },
      linux64: {
        options: {
          archive: '../trustnotebuilds/trustnote-linux64.zip'
        },
        expand: true,
        cwd: '../trustnotebuilds/Trustnote/linux64/',
        src: ['**/*'],
        dest: 'trustnote-linux64/'
      }
    },
    browserify: {
        dist:{
    ...
    debian_package: {
        linux64: {
            files: [
                {expand: true, cwd: '../trustnotebuilds/trustnote-test/linux64/', src: ['**/*'], dest: '/opt/trustnote-test/'},
                //{expand: true, cwd: '../trustnotebuilds/trustnote-test/linux64', src: ['trustnote.desktop'], dest: '/usr/share/applications/trustnote-test.desktop'}
            ],
            options: {
                maintainer: {
                    name: 'Trustnote',
                    email: 'trustnote@trustnote.org'
                },
